...
// hu 获得最新的错误信息
wchar_t * __stdcall GetLastDriverError()
{
	return lastError;
}

// hu 将驱动状态转化为字符串
wchar_t * __stdcall DriverStatus2Str(int driverStatus)
{
	switch(driverStatus)
	{
...
wchar_t * __stdcall DriverMemStatus2Str(int driverMemStatus,UINT driverMemSize)
{
...
wchar_t * __stdcall IniFpgaFlag2Str(UINT iniFpgaFlag)
{
	switch(iniFpgaFlag)
	{
...
DWORD __stdcall InitAllDSPDriver(DSPDriver *dspDriver,int driverNum,bool isStrongCorrelation)
{
	wchar_t temp[20];
	int ii;
...
// hu 获得指定驱动状态
void __stdcall GetDSPDevStatus(DSPDriver *dspDriver,int index)
{
...
// hu 刷新驱动状态
// hu 返回初始化驱动数目
int __stdcall RefreshDSPDevStatus(DSPDriver *dspDriver,int driverNum)
{
	return GetDriverStatus(dspDriver,driverNum);
}

// hu 注册消息传递句柄
void __stdcall RegistHWNDToDev(HWND hUserWnd)
{
	COPYDATASTRUCT cds;
	cds.dwData = 0;
	cds.lpData = &hUserWnd;
...
// hu 设置开机自启动
void __stdcall SetDSPAutoRun(BOOL Enable)
{
	COPYDATASTRUCT cds;
	cds.dwData = 0;
	cds.lpData = &Enable;
...
// hu 向指定DSP地址写32bit数据
// hu 0 -- 成功 1 -- PCI设备写入失败
DWORD __stdcall WriteToDSP(int deviceX,int barX, unsigned long dwoffset, unsigned int write32,unsigned int repeatTimes)
{
	COPYDATASTRUCT cds;
	unsigned int status;
...
// hu 向指定DSP地址读32bit数据
// hu 0 -- 成功 1 -- PCI设备读取失败
DWORD __stdcall ReadFromDSP(int deviceX,int barX, unsigned long dwoffset, unsigned int *read32)
{
	COPYDATASTRUCT cds;
	unsigned int status;
...
DWORD __stdcall DmaFileToDSP(int deviceX,UINT memIdx,wchar_t *binPath,WriteToDev pPhyAddr,WriteToDev dataLen,WriteToDev *endFlag,HWND hEndWnd)
{
	COPYDATASTRUCT cds;
	DmaToDev dma2Dev;
	FILE *fp;
...
DWORD __stdcall DmaDSPToFile(int deviceX,UINT memIdx,wchar_t *binPath,WriteToDev pPhyAddr,WriteToDev dataLen,ReadFromDev *endFlag,HWND hEndWnd)
{
	COPYDATASTRUCT cds;
	DmaFromDev dma2Pc;
	FILE *fp;
...
DWORD __stdcall GetMemAddr(int deviceX,UINT memIdx,UINT **pBufAddr)
{
	unsigned int status;
...
DWORD __stdcall DmaMemToDSP(int deviceX,UINT memIdx,UINT *pBufAddr,WriteToDev pPhyAddr,WriteToDev dataLen,WriteToDev *endFlag,HWND hEndWnd)
{
	COPYDATASTRUCT cds;
	DmaToDev dma2Dev;
...
// hu 1--物理空间索引不能大于等于10 2--设备号(deviceX)超过范围 3--物理空间索引超出范围 
//    4--索引物理空间状态出错 5--内存空间大小与物理空间不匹配 6--数据长度(Byte)超出范围
DWORD __stdcall DmaDSPToMem(int deviceX,UINT memIdx,UINT *pBufAddr,WriteToDev pPhyAddr,WriteToDev dataLen,HWND hEndWnd)
{
	COPYDATASTRUCT cds;
	DmaFromDev dma2Pc;
...
void __stdcall CloseAllDSPDriver()
{
...
DWORD __stdcall InitDSPDriver(int deviceX)
{
	COPYDATASTRUCT cds;
	unsigned int status;
...
DWORD __stdcall CloseDSPDriver(int deviceX)
{
	COPYDATASTRUCT cds;
	unsigned int status;
...
DWORD __stdcall ApplyForPA(int deviceX,DWORD dwDMABufSize)
{
	COPYDATASTRUCT cds;
	unsigned int status;
...
DWORD __stdcall FreePA(int deviceX,UINT memIdx)
{
	COPYDATASTRUCT cds;
	unsigned int status;
